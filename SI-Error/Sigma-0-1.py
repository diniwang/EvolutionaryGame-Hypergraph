...
for t in tList:
    ...
    a = np.trace(Pi @ np.linalg.matrix_power(Xt, t) @ Ph @ np.linalg.matrix_power(Xt, t))
    b = np.trace(Pi @ np.linalg.matrix_power(Xt, 2*t) @ Ph)
...
plt.ylabel(r"log$(\sigma^{(0,1)}) $",fontsize=18)

plt.savefig('sigma-0-1.jpg',bbox_inches = 'tight',dpi=2000)