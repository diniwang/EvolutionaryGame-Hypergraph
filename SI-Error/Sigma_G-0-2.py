...
for t in tList:
    ...
    a = np.trace(Pit @ np.linalg.matrix_power(Xt, t) @ Ph @ Ph @ np.linalg.matrix_power(Xt, t))
    b = np.trace(Pit @ np.linalg.matrix_power(Xt, 2*t)  @ Ph @ Ph)
...
plt.ylabel(r"log$(\sigma^{(0,2)}_G) $",fontsize=18)

plt.savefig('sigma_G-0-2.jpg',bbox_inches = 'tight',dpi=2000)