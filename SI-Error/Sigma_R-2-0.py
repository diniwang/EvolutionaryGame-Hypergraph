...
for t in tList:
    ...
    a = np.trace(Pi @ np.linalg.matrix_power(Xt, t) @ Pt @ Pt @ np.linalg.inv(K) @ B @ B.T @ np.linalg.matrix_power(Xt, t))
    b = np.trace(Pi @ np.linalg.matrix_power(Xt, 2*t) @ Pt @ Pt @ np.linalg.inv(K) @ B @ B.T)
...
plt.ylabel(r"log$(\sigma^{(2,0)}_R) $",fontsize=18)

plt.savefig('sigma_R-2-0.jpg',bbox_inches = 'tight',dpi=2000)