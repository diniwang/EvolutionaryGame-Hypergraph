...
for t in range(TestNum):
    ...
    for con in range(ContectNum):
        ...
        # The mean fitness of incident hyperedge except the selected vertex
        fit_edge_lst = []
        for e in Nei_dict[i]:
            fit_edge_ex = np.mean([Compute_fitness(ii, strategy, b, c) for ii in E[e] if ii != i])
            fit_edge_lst.append(fit_edge_ex * W[e])
        ...
        for ii in range(len(fit_prob)):
            if rand01 < fit_prob[ii]:
                ...
                fit_total = fit_edge_lst[ii] * (len(E[selected_l]) - 1) / W[selected_l]
                break
            
        # Probability of selecting the strategy C
        fit_C = sum([Compute_fitness(ii, strategy, b, c)*strategy[ii] for ii in E[selected_l] if ii != i])
        tran = fit_C / fit_total
